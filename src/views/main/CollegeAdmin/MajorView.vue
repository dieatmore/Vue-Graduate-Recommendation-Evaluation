<template>
  <div class="w-full main-container bg-[#F9FAFB] py-4 px-8">
    ...
    <el-dialog
      ...
      :title="dialogStatusR == 'add' ? '添加专业' : '修改专业'"
      @close="handleCloseF"
      width="400">
...
const { data: majorListR } = CollegeAdmin.getMajorsByCatIdService(catIdR) // 获取专业列表

const addFormR = ref({
  name: ''
})

const addMajorMutation = CollegeAdmin.addMajorService(catIdR) // 添加专业
const updateMajorMutation = CollegeAdmin.updateMajorService(catIdR) // 修改专业
const deleteMajorMutation = CollegeAdmin.deleteMajorService(catIdR) // 删除专业

// 打开类别dialog
const openDialogF = (status: string, data?: Major) => {
...
const handleConfirmF = async () => {
  ...
  if (dialogStatusR.value === 'edit') {
    await updateMajorMutation.mutateAsync({ majorId: editId.value, major: addFormR.value })
    dialogFormVisibleR.value = false
    message.success('修改成功！')
    return
  } else {
    await addMajorMutation.mutateAsync(addFormR.value)
    dialogFormVisibleR.value = false
